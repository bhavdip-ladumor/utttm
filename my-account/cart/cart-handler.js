...
/**
 * HELPER: PATH & BRAND DETECTOR
 * Maps the product brand or URL path to a specific Store ID
 */
const getStoreId = (product) => {
    // 1. HIGHEST PRIORITY: Check the URL Path (Where is the user right now?)
    const path = window.location.pathname.toLowerCase();
    
    if (path.includes('/resin/')) return "Resin-cosmos-store";
    if (path.includes('/wooden/')) return "Wooden-Beyond-store";
    if (path.includes('/acrylic/')) return "Acrylic-Fortune-store";

    // 2. SECOND PRIORITY: Check Brand (Only if URL check fails)
    const brandMap = {
        "Resin cosmos": "Resin-cosmos-store",
        "Wooden Beyond": "Wooden-Beyond-store",
        "acrylic fortune": "Acrylic-Fortune-store"
    };

    if (product.brand && brandMap[product.brand]) {
        return brandMap[product.brand];
    }
...
export function addToCart(product) {
    ...
    if (existingItem) {
        ...
        cart.push({
            ...
            name: product.name,
            storeId: getStoreId(product), // NEW: Attach Store ID here
        ...
        <div class="cart-item">
            <a href="${productLink}" class="cart-item-link">
                ...
                <div class="cart-details">
                    <p class="cart-store-tag">${item.brand}</p>

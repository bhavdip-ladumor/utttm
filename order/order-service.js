...
document.getElementById('checkout-form').addEventListener('submit', async (e) => {
    ...
    // A. Collect all data from the form
    const formData = {
    ...
    // B. SAVE TO LOCALSTORAGE (Required for the WhatsApp message)
    localStorage.setItem('user_checkout_details', JSON.stringify(formData));

    try {
        // C. CREATE THE LEAD (No "if auth" needed here)
        // This will now write to the 'leads' collection for EVERYONE
        const orderId = await window.processFinalOrder();

        if (orderId) {
            // D. OPTIONAL: Save to user profile ONLY if they happen to be logged in
            if (auth.currentUser) {

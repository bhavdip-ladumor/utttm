...
function renderStaticUI() {
    ...
    document.title = `${selectedVariant.name} | woodenbeyond`;
    document.getElementById('product-name').innerText = selectedVariant.name;
    document.getElementById('product-brand').innerText = `By ${selectedVariant.brand || 'woodenbeyond'}`;
...
// --- 3. ADD THE RESET CODE HERE ---
    ...
    const msg = encodeURIComponent(`Hi Wooden Beyond! I'm interested in: ${selectedVariant.name} (${selectedVariant.attrValue}). SKU: ${selectedVariant.sku}`);
...
if (mainCartBtn) {
    mainCartBtn.onclick = () => { // Changed to onclick for cleaner overwriting
        if (selectedVariant && typeof window.addToCart === 'function') {
            // This sends the currently SELECTED variant object to the cart
            window.addToCart(selectedVariant); 
            console.log("Added variant to cart:", selectedVariant.sku);
        } else {
            console.error("Cart function not found or variant not selected");
        }
    };
}

// Fix for Similar Products "Add" buttons
